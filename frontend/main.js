...
(async function () {
    ...
    let serverConfig;
    try {
        serverConfig = await getServerConfigInfo();
        debugger;
    } catch (err) {
    ...
    const currentObjectView = localStorage['currentObjectView'];
    ...
    if (!currentObjectView) {
        ...
        let menuData = await getMenuInfoFromServer(currentObjectView);
        buildMenu(menuData);
    }
